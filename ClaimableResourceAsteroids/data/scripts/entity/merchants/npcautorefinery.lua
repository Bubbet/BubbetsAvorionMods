...
function NpcAutoRefinery.processResources(timeStep)
	...
	local is_hub = entity:getValue("station_hub_id") -- if no hub then nil and fail the following
	if is_hub then
		...
		if is_hub then entity = is_hub end
	end
	--print(hub, 'hub')
	for i=0,NumMaterials()-1 do
		i = NumMaterials()-1 - i -- make it process avorion first
		if materialcount == 0 then return end
		local ore = entity:findCargos(oreNameByMaterial[i+1])
		local scrap = entity:findCargos(scrapNameByMaterial[i+1])
