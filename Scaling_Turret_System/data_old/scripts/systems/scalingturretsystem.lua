...
function getTooltipLines(seed, rarity, permanent)
    ...
    local lines = {{icon = "data/textures/icons/turret.png", ltext = stattype .. " Slots", rtext = "+??", boosted = permanent},
                   ...
                   {},
                   {ltext = "Ship Stats Used:"}}
    for k, v in pairs(Config.stats) do
        local display = v * (rarity.value + 2) / 10
        if permanent then display = display * 1.25 end
        table.insert(lines, {ltext = '* ' .. k:firstToUpper() .. " at a weight of " .. display, lcolor = permanent and ColorRGB(0, 1, 0)})
        sum = sum + v
    end
...
function getDescriptionLines(seed, rarity, permanent)
    return
    {
        ...
        {ltext = "Actual stats gained may not represent"},
        {ltext = "weights 100% due to the server"},
        {ltext = "using a different scaling function."},
